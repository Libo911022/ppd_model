...
BASE_DIR = os.path.join(dirname(dirname(dirname(abspath(__file__)))), 'conf_ppd')
...
class Config(object):
    ...
    def _check_feature_conf(feature, valid_feature_name, **kwargs):
        ...
        if type_ == 'category':
            ...
            if trans is not None:
                ...
                if trans == 'min_max' or 'standard' and normalization is not None:
                ...
                if trans == 'min_max' and normalization is not None:
                ...
                elif trans == 'standard' and normalization is not None:
    ...
    @staticmethod
    def _check_dict(key, value):
        if not isinstance(value, dict):
            raise ValueError('dict type is required for key `{}`, found `{}`.'.format(key, value))
    ...
    def _read_train_conf(self):
        ...
        opt_num_keys = ['num_parallel_calls']
        dict_num_keys = ['loss_weight']
        ...
        with open(self._train_conf_file) as f:
            ...
            for k, v in train_conf['train'].items():
                if k in req_str_keys:
                    ...
                    self._check_required(k, v)
                    self._check_bool(k, v)
                elif k in dict_num_keys:
                    self._check_dict(k, v)
            return train_conf
    ...
    def get_feature_name(self, feature_type='all'):
        ...
        schema_dict = self.read_schema()
        schema_dict.pop(1) #remove label,label must be first
        feature_list = schema_dict.values()
        if feature_type == 'all':
            return feature_list
