...
from lib.model_util import activation_fn
        ...
        if f_type == 'category':
            if f_tran == 'hash_bucket':
                ...
                col = categorical_column_with_vocabulary_list(feature,
                    ...
                    dtype=None)  # len(vocab)+num_oov_buckets
                wide_columns.append(col)
            ...
            try:
                normalization = f_param['normalization']
            except:
                normalization = None

            try:
                boundaries = f_param['boundaries']
            except:
                boundaries = None

            # normalizaton, boundaries = f_param["normalization"], f_param["boundaries"]
            if f_tran is not None and normalization is not None:
                normalizer_fn = normalizer_fn_builder(f_tran, tuple(normalization))
            else:
                normalizer_fn = None
            ...
            if boundaries is not None:  # whether include continuous features in wide part
...
def build_estimator(model_dir, model_type):
    ...
    run_config = tf.estimator.RunConfig().replace(**CONF.runconfig)#.replace(session_config=tf.ConfigProto(device_count={'GPU': 0}))
    if model_type == 'wide':
...
#         session_config=tf.ConfigProto(device_count={'GPU': 0}))
    run_config = tf.estimator.RunConfig().replace(**CONF.runconfig)
